...
px=[color]*800
isdarkpalette=True
print("\x1b[33;1m\x1b[1mIZANAGI Linux Edition\nCreated by Katsumi Kougen ~ github.com/KatsumiKougen\x1b[0m")
...
def initTerm():
	...
	maxy,maxx=mainscreen.getmaxyx()[1],mainscreen.getmaxyx()[0]

def drawMenu():
	mainscreen.clear()
	mainscreen.addstr(0,0,"IZANAGI Ver 2.1 - (C)2021 Katsumi Kougen. All rights reserved.",curses.color_pair(3))
	mainscreen.addstr(1,0,".",curses.color_pair(3))
	for x in range(1,41):
		mainscreen.addstr(1,x,"-",curses.color_pair(3))
	mainscreen.addstr(1,41,".",curses.color_pair(3))
	for y in range(2,22):
		mainscreen.addstr(y,0,"|",curses.color_pair(3))
		for x in range(1,41):
			mainscreen.addstr(y,x," ",curses.color_pair(60))
		mainscreen.addstr(y,41,"|",curses.color_pair(3))
	mainscreen.addstr(22,0,"'",curses.color_pair(3))
	for x in range(1,41):
		mainscreen.addstr(22,x,"-",curses.color_pair(3))
	mainscreen.addstr(22,41,"'",curses.color_pair(3))
	mainscreen.addstr(23,0,splash[r.randrange(len(splash))],curses.color_pair(2))
	for indx,string in enumerate(prginstString):
		mainscreen.addstr(indx+1,42,string,curses.color_pair(2))
	printPaletteType()
	update()
